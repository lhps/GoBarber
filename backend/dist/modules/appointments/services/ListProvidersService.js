...
let ListProvidersService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    let users = await this.cacheProvider.recover(`provider-list:${user_id}`);

    if (!users) {
      users = await this.usersRepository.findAllProviders({
        except_user_id: user_id
      });
      await this.cacheProvider.save(`provider-list:${user_id}`, (0, _classTransformer.classToClass)(users));
    }
